...
const _supabase_1 = require("./_supabase");
async function syncGitHubPRs(ghToken, userId) {
    ...
    for (const item of searchData.items.slice(0, 50)) {
        ...
        try {
            ...
            await (0, _supabase_1.getSupabase)().from('github_prs').upsert({
                user_id: userId,
                pr_number: pr.number,
...
async function syncFigmaFrames(figmaToken, userId) {
    let synced = 0;
    ...
    for (const file of fileKeys.slice(0, 20)) {
        try {
            const fileRes = await fetch(`https://api.figma.com/v1/files/${file.key}?depth=3`, { headers: { Authorization: `Bearer ${figmaToken}` } });
            if (!fileRes.ok)
                continue;
            ...
            if (frames.length > 0) {
                ...
                try {
                    const thumbRes = await fetch(`https://api.figma.com/v1/images/${file.key}?ids=${encodeURIComponent(ids)}&format=png&scale=1`, { headers: { Authorization: `Bearer ${figmaToken}` } });
                    if (thumbRes.ok) {
            ...
            for (const frame of frames) {
                await (0, _supabase_1.getSupabase)().from('figma_frames').upsert({
                    user_id: userId,
                    file_key: file.key,
                    file_name: file.name,
                    frame_id: frame.id,
...
async function handler(req, res) {
    ...
    try {
        ...
        // Upsert user record
        const { data: userData } = await (0, _supabase_1.getSupabase)()
            .from('users')
            .upsert({
